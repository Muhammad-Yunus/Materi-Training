...
class Postprocessing :
    ...
    def transformX(self):
        ...
        for channel in self.glcm_feature_list: 
            ...
            self.x2.append(sample)
            
        # concate 2d Matrix
        self.x1 = np.array(self.x1).astype(np.float32)
        self.x2 = np.array(self.x2).astype(np.float32)
        self.X = np.concatenate((self.x1, self.x2), axis=1)
        print("X size :\n", self.X.shape)
...
class TrainingMLP:
    def __init__(self, X_train, y_train, X_test, y_test, labels_vec, labels_name, max_iteartion=100, min_accuracy=0.9, input_dim=2):
        ...
        network_layer = np.array([input_dim, 128, 64, 32, output_dim]).astype(np.uint8)
