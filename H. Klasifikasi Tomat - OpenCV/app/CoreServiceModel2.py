...
#############################################################################
class TrainingMLP:
    def __init__(self, X_train, y_train, X_test, y_test, labels_vec, labels_name, max_iteartion=100, min_accuracy=0.9, input_dim=2, model_name="Model2"):
...
#############################################################################
class Prediction:
    def __init__(self, labels_name, model_name = "Model2"):
    ...
    def predict(self, X_list):
        if len(X_list) <= 1 :
            raise "need more than 1 image!"
        
        y_proba_list = []
        y_label_list = []
        for X in X_list:
            y = self.mlp.predict(np.array([X]))[1]
            y_proba = max(min(y.max()*100, 100), 0)
            y_proba_list.append(y_proba)
            y_label = self.labels_name[y.argmax(axis=1)[0]]
            y_label_list.append(y_label)
        
        isGood = all([True if item == self.labels_name[0] else False for item in y_label_list])
        y_label_final = self.labels_name[1 - int(isGood)]
        y_proba_final = np.array(y_proba_list).mean()
        self.output  = "%s (%d%%)" % (y_label_final, y_proba_final)
